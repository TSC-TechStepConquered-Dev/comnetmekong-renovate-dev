...
const menuItems = [
  ...
  { name: 'การบริจาค', path: ROUTES.DONATE },
]
...
<template>
  <header class="absolute top-0 left-0 w-full z-50 px-4 py-6 md:px-8">
    <div class="max-w-7xl mx-auto relative">
      <!-- Main Header Pill -->
      <div
        class="relative z-20 flex items-center justify-between bg-black/20 backdrop-blur-md border border-white/10 rounded-full px-6 py-3 shadow-lg transition-colors duration-300"
      >
        <!-- Logo Area -->
        <RouterLink :to="ROUTES.HOME" class="flex items-center gap-3 group shrink-0">
          <div
            class="w-10 h-10 bg-white rounded-full flex items-center justify-center overflow-hidden shrink-0"
          >
            <img
              src="../../../assets/logo_2.avif"
              alt="Hugmekong Logo"
          ...
          <div class="flex flex-col">
            <span
              class="text-white font-bold tracking-wider text-sm group-hover:text-amber-300 transition-colors"
              >COMNETMEKONG</span
            >
            <span
              class="text-white/70 text-[10px] tracking-widest uppercase truncate hidden sm:block"
              >Stories along the river</span
            >
          </div>
        </RouterLink>
        ...
        <nav class="hidden lg:flex items-center gap-0">
          <div v-for="item in menuItems" :key="item.name">
            <RouterLink
              ...
              :class="
                route.path === item.path ? 'text-amber-300' : 'text-white/90 hover:text-white'
              "
            >
              {{ item.name }}
            </RouterLink>
        ...
        <div class="flex items-center gap-2">
          ...
          <template v-if="authStore.isAuthenticated && authStore.user">
            <div
              class="relative profile-dropdown-wrapper flex items-center gap-2 cursor-pointer bg-white/10 hover:bg-white/20 border border-white/20 rounded-full pl-2 pr-1.5 py-1.5 transition-all"
              @click.stop="toggleProfileDropdown"
            >
              <span class="text-white text-sm font-medium hidden sm:block ml-2">{{
                authStore.user.displayName
              }}</span>
              <div
                class="flex items-center justify-center w-8 h-8 rounded-full bg-amber-600 text-white shadow-sm"
              >
                <svg
                  class="w-4 h-4"
                  fill="none"
              ...
              <Transition name="dropdown-fade">
                <div
                  ...
                  class="hidden lg:block absolute right-0 top-full mt-2 w-48 z-50"
                >
                  <div
                    class="bg-white rounded-xl shadow-xl border border-stone-100 py-2 overflow-hidden"
                  >
                    <button
                      @click.stop="openProfileModal"
                      class="w-full text-left block px-4 py-2.5 text-sm text-stone-700 hover:bg-stone-50 transition-colors"
                    >
                      จัดการโปรไฟล์
                    </button>
                    <button
          ...
          <button
            ...
            aria-label="เปิดเมนู"
            class="lg:hidden flex items-center justify-center w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 transition-all text-white"
          >
            <svg
              v-if="!mobileMenuOpen"
              class="w-5 h-5"
      ...
      >
        <div class="p-6 space-y-4">
          ...
          <div
            ...
            class="pb-4 mb-4 border-b border-stone-200"
          >
            ...
            <div class="flex gap-2">
              <button
                ...
                class="flex-1 text-center px-4 py-2 text-sm bg-stone-100 text-stone-700 rounded-full font-medium hover:bg-stone-200"
              >
                จัดการโปรไฟล์
              </button>
              <button
                ...
                "
                class="flex-1 px-4 py-2 text-sm bg-red-50 text-red-600 rounded-full font-medium hover:bg-red-100"
              >
                ออกจากระบบ
              </button>
            </div>
          ...
          <RouterLink
            ...
            :class="
              route.path === item.path ? 'text-amber-600' : 'text-stone-700 hover:text-amber-600'
            "
    ...
    <div v-if="showLogoutModal" class="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      >
        ...
        <p class="text-center text-stone-600 mb-8 leading-relaxed">
        ...
        <div class="flex gap-3">
          ...
          <button
            @click="confirmLogout"
            class="flex-1 px-4 py-3 rounded-xl bg-amber-700 text-white font-medium hover:bg-amber-800 transition-colors shadow-sm"
          >
            ออกจากระบบ
          </button>
        </div>

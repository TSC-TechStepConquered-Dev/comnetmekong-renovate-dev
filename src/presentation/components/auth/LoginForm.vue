...
const handleLogin = async () => {
  ...
  if (result.success) {
    ...
    errorMessage.value = result.error || 'การเข้าสู่ระบบล้มเหลว'
  }
}
...
const handleGoogleCallback = async (response) => {
  try {
    ...
    if (result.success) {
      // ลองใช้ window.location.href เผื่อว่า router.push มีปัญหา
...
<template>
  <div class="w-full max-w-md p-8 space-y-6 bg-white/70 backdrop-blur-md rounded-2xl shadow-xl border border-white/20">
    <div class="text-center">
      <h2 class="text-3xl font-bold text-stone-800 tracking-tight">เข้าสู่ระบบ</h2>
      <p class="mt-2 text-sm text-stone-600">
        ยินดีต้อนรับกลับสู่ HUGMEKONG
      </p>
    </div>
    ...
    <form class="space-y-5" @submit.prevent="handleLogin">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center">
          <input id="remember-me" name="remember-me" type="checkbox" class="h-4 w-4 text-amber-700 focus:ring-amber-600 border-stone-300 rounded">
          <label for="remember-me" class="ml-2 block text-sm text-stone-600">จดจำฉันไว้ในระบบ</label>
        </div>
        <div class="text-sm">
          <a href="#" class="font-medium text-amber-700 hover:text-amber-600 transition-colors">ลืมรหัสผ่าน?</a>
        </div>
      </div>
    ...
    <div class="text-center text-sm text-stone-600">
      ...
      <RouterLink :to="ROUTES.AUTH.REGISTER" class="font-medium text-amber-700 hover:text-amber-600 transition-colors">
        สมัครสมาชิก
      </RouterLink>
    </div>
  </div>

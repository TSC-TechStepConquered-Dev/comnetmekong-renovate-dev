...
// MOCK DATA: มี field "gallery" เป็น array ของรูปภาพ
const MOCK_YOUTH_DATA = [
  {
    id: 1,
    title: 'กิจกรรมค่ายเยาวชนอนุรักษ์ลุ่มน้ำโขง 2567',
    date: '20-22 สิงหาคม 2567',
    description: `เครือข่ายลุ่มน้ำโขงได้จัดกิจกรรมเพื่อส่งเสริมความรู้ความเข้าใจให้กับเยาวชนคนรุ่นใหม่เกี่ยวกับความสำคัญของระบบนิเวศและวัฒนธรรมท้องถิ่น\n\nผู้เข้าร่วมกิจกรรมได้มีโอกาสลงพื้นที่จริง เรียนรู้วิถีชีวิตของชาวประมงพื้นบ้าน และศึกษาสายพันธุ์ปลาที่ได้รับผลกระทบจากการเปลี่ยนแปลงของระดับน้ำ`,
  ...
  {
    id: 2,
    title: 'โครงการผู้นำเยาวชนพิทักษ์สายน้ำ',
    date: '5 กันยายน 2567',
    description: `พัฒนาศักยภาพเยาวชนในพื้นที่ให้เป็นกระบอกเสียงและเป็นผู้นำในการรณรงค์แก้ไขปัญหาสิ่งแวดล้อมระดับชุมชน โดยมีการอบรมเชิงปฏิบัติการด้านการสื่อสารและการทำวิจัยจิ๋ว (Mini-research)`,
    videoUrl: '',
    gallery: [
...
const fetchData = async () => {
  try {
    ...
    if (!response || !response.ok) {
      youthData.value = MOCK_YOUTH_DATA
      return
    }
    ...
    if (Array.isArray(items) && items.length > 0) {
      youthData.value = items.map(item => {
    ...
    console.warn('Error fetching mekong youth data, using mock', error)
    youthData.value = MOCK_YOUTH_DATA
  } finally {
    ...
    <section class="relative z-20 w-full bg-[#f4f1ea] py-24 px-4 md:px-8 pb-32">
      <div class="max-w-5xl mx-auto flex flex-col gap-16">
        ...
        <div class="w-full text-center" data-aos="fade-up">
          <h2 class="text-4xl md:text-5xl lg:text-6xl font-black text-stone-900 tracking-tighter flex justify-center items-baseline gap-1">
        ...
        <div v-if="loadingData" class="space-y-24 mt-8">
          <div v-for="i in 2" :key="i" class="animate-pulse bg-white rounded-3xl p-8 border border-stone-200 shadow-sm">
        ...
        <div v-else class="space-y-20 mt-8">
          ...
          >
            ...
            <!-- Description -->
            <div class="text-stone-700 leading-relaxed text-base md:text-lg mb-10 text-justify md:text-left">
              <div v-html="item.description" class="space-y-4"></div>
            </div>
            
            <!-- Image Gallery -->
            <div v-if="item.gallery && item.gallery.length > 0">
              <h4 class="text-sm font-bold text-stone-400 tracking-widest uppercase mb-4">Gallery</h4>
      ...
      <div class="max-w-7xl mx-auto flex flex-col gap-16">
        ...
        <div v-if="!loadingData" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
          <template v-for="(item, index) in youthData" :key="`video-${item.id || index}`">
            <div v-if="item.videoUrl" class="bg-[#f4f1ea] rounded-3xl p-4 shadow-sm border border-stone-100 hover:shadow-md transition-shadow" data-aos="fade-up">

...
<template>
  <main class="min-h-screen bg-[#f4f1ea] font-sans">
    ...
    >
      ...
      >
        <h1
          class="text-5xl md:text-7xl lg:text-8xl font-black text-white tracking-tighter uppercase drop-shadow-lg"
        >
    ...
    <div class="pt-12 pb-16 px-4 md:px-8 max-w-7xl mx-auto relative z-30">
      ...
      <div class="text-center mb-12">
        <h2 class="text-2xl md:text-4xl font-bold font-serif mb-3 text-[#332f2c]">
          บริจาคเพื่อแม่น้ำโขง
        </h2>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-6 md:gap-10 items-start">
        ...
        <div class="space-y-6">
          <!-- QR Code Card -->
          <div
            class="bg-white rounded-[2rem] p-8 md:p-12 shadow-sm flex flex-col items-center text-center"
          >
            ...
            <div
              class="w-56 h-56 md:w-72 md:h-72 bg-white border-2 border-stone-100 rounded-3xl p-4 flex items-center justify-center mb-6 overflow-hidden"
            >
              <img
              ...
              <div
                v-else
                class="w-full h-full flex items-center justify-center bg-stone-50 rounded-2xl"
              >
                <!-- Fallback QR icon -->
                <svg class="w-24 h-24 text-stone-300" fill="currentColor" viewBox="0 0 24 24">
                  <path
                    d="M3 3h8v8H3zM5 5v4h4V5zM13 3h8v8h-8zM15 5v4h4V5zM3 13h8v8H3zM5 15v4h4v-4zM18 13h3v3h-3zM13 18h3v3h-3zM15 15h3v3h-3zM18 18h3v3h-3zM13 13h3v3h-3z"
                  />
                </svg>
              </div>
            </div>

            <h3 class="font-bold text-lg uppercase tracking-wider mb-2">
              {{ donateInfo.orgName || 'COMNETMEKONG' }}
            </h3>
            <p class="text-stone-600 text-sm font-medium">
              PromptPay: <span class="font-bold">{{ donateInfo.promptpay }}</span>
            </p>
            <p class="text-stone-500 text-xs mt-2 max-w-[80%] mx-auto">
          ...
          <!-- Bank Details Card -->
          <div
            class="bg-[#ebe6db] border border-stone-200/50 rounded-[2rem] p-6 md:p-8 flex flex-col gap-5 shadow-sm"
          >
            ...
            <div class="flex justify-between items-end border-b border-stone-300/50 pb-3">
              ...
              <span class="text-xs md:text-sm font-bold text-stone-800 text-right max-w-[65%]">{{
                donateInfo.accountName
              }}</span>
            </div>
            ...
            <div class="flex justify-between items-center pt-2">
              ...
              <span class="text-lg md:text-xl font-black text-stone-900 tracking-widest">{{
        ...
        <div class="bg-white rounded-[2rem] p-8 md:p-12 shadow-sm">
          ...
          <form @submit.prevent="submitDonation" class="space-y-6">
            <!-- Name -->
            <div>
              <label class="block text-[13px] font-bold text-stone-800 mb-2"
                >ชื่อ-นามสกุล (ไม่บังคับ)</label
              >
              <input
                type="text"
                v-model="form.name"
            ...
            <div>
              ...
              ></textarea>
              <div class="text-right mt-1 text-[11px] text-stone-400">
            ...
            <!-- Slip Upload -->
            <div class="pt-2">
              <label class="block text-[13px] font-bold text-stone-800 mb-3"
                >แนบสลิปการโอนเงิน / หลักฐาน</label
              >
              ...
              <div v-if="!slipPreview" class="relative group">
                ...
                />
                <div
                  class="bg-[#fcfbf9] border-2 border-dashed border-stone-200 rounded-2xl p-10 flex flex-col items-center justify-center text-center group-hover:border-stone-300 group-hover:bg-[#f6f4f0] transition-all"
                >
                  <div
                    class="w-14 h-14 bg-stone-100 rounded-full flex items-center justify-center mb-4 text-stone-400 group-hover:scale-110 transition-transform"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      class="h-6 w-6"
              ...
              <div v-else class="relative bg-stone-50 rounded-2xl p-3 border border-stone-200">
                <img
                  :src="slipPreview"
                  alt="Slip Preview"
                  class="w-full h-56 object-contain rounded-xl bg-white"
                />
                <button
                  type="button"
                  @click="removeSlip"
            ...
            <div
              v-if="submitSuccess"
              class="bg-green-50 text-green-700 p-4 rounded-xl text-sm font-medium border border-green-200 mt-4"
            >
              ขอบคุณสำหรับการสนับสนุน! ทางเราได้รับหลักฐานการโอนเงินเรียบร้อยแล้ว
            </div>
            <div
              v-if="submitError"
              class="bg-red-50 text-red-700 p-4 rounded-xl text-sm font-medium border border-red-200 mt-4"
            >
              {{ submitError }}
            </div>

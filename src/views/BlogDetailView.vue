...
const submitComment = async () => {
  ...
  try {
    ...
    const response = await blogRepo.addComment(blog.value.id, authStore.token, commentContent.value, authorName)
    ...
    if (response && response.data) {
      ...
      blog.value.comments.push({
        ...
        date: new Date(response.data.createdAt).toLocaleDateString('th-TH', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })
      })
    } else {
...
<template>
  <main class="min-h-screen bg-[#f4f1ea] font-sans">
    ...
    <div v-else-if="blog" class="max-w-4xl mx-auto px-4 md:px-8 pt-32 animate-fade-in">
      ...
      <!-- Breadcrumb & Category -->
      <div class="flex items-center gap-3 text-sm text-amber-600 font-semibold tracking-widest uppercase mb-6">
      ...
      <!-- Meta Info -->
      <div class="flex flex-wrap items-center gap-6 text-sm text-stone-500 mb-8 pb-8 border-b border-stone-200">
      ...
      <div class="w-full h-[300px] md:h-[500px] rounded-3xl overflow-hidden mb-12 shadow-sm">
        <img :src="blog.imageUrl" :alt="blog.title" loading="lazy" decoding="async" class="w-full h-full object-cover">
      </div>

      <!-- Content -->
      <div class="prose prose-stone prose-lg max-w-none text-stone-700 leading-relaxed mb-16" v-html="blog.content">
      </div>
      ...
      <div class="flex items-center justify-between py-6 border-t border-b border-stone-200 mb-16">
        <div class="flex items-center gap-4">
          <button 
            ...
            :class="isLiked ? 'bg-amber-50 text-amber-600 border-amber-200' : 'bg-white text-stone-600 border-stone-200 hover:bg-stone-50'"
          >
            <!-- Broken Heart (Unliking) -->
            <svg v-if="isLiked && isLiking" class="w-6 h-6 animate-pulse text-stone-400" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09l-2 3.91 2 2.5-2 2.5 2 3.5-2 3.85z" />
              <path d="M12 21.35l2-3.85-2-3.5 2-2.5-2-2.5 2-3.91C15.09 3.81 16.76 3 18.5 3 21.58 3 24 5.42 24 8.5c0 3.78-3.4 6.86-8.55 11.53L14 21.35z" />
            </svg>
            <!-- Normal Heart (Liked, Liking, or Unliked) -->
            <svg v-else class="w-6 h-6 transition-transform" :class="{'animate-heartbeat text-red-500': isLiking && !isLiked}" :fill="isLiked || (isLiking && !isLiked) ? 'currentColor' : 'none'" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button @click="handleShare" class="flex items-center gap-2 px-6 py-3 rounded-full bg-white text-stone-600 border border-stone-200 hover:bg-stone-50 transition-colors font-medium">
      ...
      <!-- Comments Section -->
      <div class="mb-12">
        <h3 class="text-2xl font-bold text-stone-800 mb-8">ความคิดเห็น ({{ blog.comments ? blog.comments.length : 0 }})</h3>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-stone-100 mb-10">
          ...
          <div v-else class="text-center py-6">
            ...
            <RouterLink to="/login" class="inline-block px-6 py-2.5 bg-stone-800 text-white font-medium rounded-xl hover:bg-stone-900 transition-colors">
              เข้าสู่ระบบ
            </RouterLink>
          </div>
        </div>
        ...
        <div v-if="blog.comments && blog.comments.length > 0" class="space-y-6">
          ...
          >
            ...
            <div class="w-12 h-12 flex-shrink-0 rounded-full bg-stone-200 text-stone-500 flex items-center justify-center font-bold text-lg">
            ...
            <div class="flex-grow min-w-0">
              <div class="flex items-center justify-between mb-1">
                ...
                <div 
                  v-if="authStore.isAuthenticated && authStore.user && comment.authorName === authStore.user.displayName && deletingCommentId !== comment.id"
                ...
                >
                  ...
                  <Transition name="menu-fade">
                    ...
                    >
                      ...
                      >
                        <svg class="w-4 h-4 text-stone-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                        แก้ไข
                      </button>
                      <button 
                        @click.stop="activeMenuId = null; deleteComment(comment.id)"
              ...
              <!-- Normal display -->
              <p v-if="editingCommentId !== comment.id" class="text-stone-600 leading-relaxed">{{ comment.content }}</p>
              <!-- Inline edit mode -->
        ...
        <div v-else class="text-center text-stone-500 py-8 bg-white rounded-2xl border border-stone-100">
      ...
      <div v-if="relatedBlogs.length > 0" class="mt-24 pt-16 border-t border-stone-200">
        <div class="flex items-center justify-between mb-8">
          ...
          <RouterLink to="/blogs" class="text-xs font-bold tracking-widest uppercase text-amber-600 hover:text-amber-700 transition-colors flex items-center gap-1">
            ดูทั้งหมด <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
          </RouterLink>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 pb-8">
          ...
          >
            <div class="relative h-48 overflow-hidden bg-stone-100">
              <img :src="rb.imageUrl" :alt="rb.title" loading="lazy" decoding="async" class="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700 ease-out">
            </div>
            <div class="p-6 flex flex-col flex-grow">
              <div class="flex items-center gap-3 text-[10px] font-semibold text-stone-500 mb-3 uppercase tracking-wider">
                <span class="text-amber-600">{{ rb.category }}</span>
                <span>•</span>
                <span>{{ rb.date }}</span>
              </div>
              <h3 class="text-lg font-bold text-stone-800 leading-tight mb-3 group-hover:text-amber-700 transition-colors line-clamp-2">
                {{ rb.title }}
              </h3>
    ...
    <div v-if="modalState.show" class="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div class="relative bg-[#f4f1ea] rounded-3xl shadow-2xl p-8 max-w-sm w-full mx-4 animate-scale-up border border-stone-200">
        ...
        <div 
          ...
          :class="{
            'bg-amber-100 text-amber-600': modalState.type === 'login' || modalState.type === 'alert',
            'bg-red-100 text-red-600': modalState.type === 'confirm'
          }"
        >
          <!-- Info/Login Icon -->
          <svg v-if="modalState.type === 'login' || (modalState.type === 'alert' && modalState.title !== 'ผิดพลาด')" class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
          <!-- Error Icon -->
          <svg v-else-if="modalState.type === 'alert' && modalState.title === 'ผิดพลาด'" class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
          <!-- Warning/Confirm Icon -->
          <svg v-else-if="modalState.type === 'confirm'" class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
        </div>

        <h3 class="text-2xl font-bold text-center text-stone-800 mb-3 tracking-tight">{{ modalState.title }}</h3>
        ...
        <div class="flex gap-3">
          ...
          <button 
            ...
            :class="{
              'bg-amber-700 hover:bg-amber-800': modalState.type === 'login' || modalState.type === 'alert',
